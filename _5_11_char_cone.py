...
import _5_11_char_cone_config as local_config
...
plot_vector(ax, local_config.ORIGIN, X[0], **global_config.PATH_TWO_PROPERTIES)
plot_vector(ax, local_config.ORIGIN, X[1], **global_config.PATH_TWO_PROPERTIES)
plot_vector(ax, local_config.ORIGIN, X[2], **global_config.PATH_TWO_PROPERTIES)
plot_point(ax, X[0], '$\\in X$', False)
plot_point(ax, X[1], '$\\in X$', False)
plot_point(ax, X[2], '$\\in X$', False)

# This way we have P = conv(X) + ccone(Y)
